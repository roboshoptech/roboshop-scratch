...
// import type { ControlFlowInLoopBlock } from "blockly/blocks/loops";

export function controls_repeat_ext(block: Block, generator: ArduinoGenerator) {
  // Repeat n times.
  let repeats;
...
export function controls_whileUntil(block: Block, generator: ArduinoGenerator) {
  // Do while/until loop.
...
export function controls_for(block: Block, generator: ArduinoGenerator) {
  // For loop.
...
export function controls_forEach(block: Block, generator: ArduinoGenerator) {
  // For each loop.
  const variable0 = generator.getVariableName(block.getFieldValue("VAR"));
  const argument0 =
