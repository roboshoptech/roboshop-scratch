...
export function text_append(block: Block, generator: ArduinoGenerator) {
...
export function text_isEmpty(
  ...
  const text = generator.valueToCode(block, "VALUE", Order.MEMBER) || `""`;
  return ["(" + text + ".length() == 0)", Order.LOGICAL_NOT];
}
...
export function text_charAt(
  ...
  switch (where) {
    ...
    case "FROM_END": {
      ...
      const code = text + ".substring(" + at + ").charAt(0)";
      return [code, Order.FUNCTION_CALL];
    }
    case "RANDOM": {
...
export function text_print(block: Block, generator: ArduinoGenerator) {
  // Print statement.
